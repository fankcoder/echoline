...
import type { Cue, DictionaryEntry, DictionarySearchDirection, DictionarySearchResult, FavoriteExample, VocabularyItem } from '../types';
...
type Props = {
  ...
  vocabulary: VocabularyItem[]; favoriteExamples: FavoriteExample[]; favoriteCueIds: Set<string>; favoriteSavingCueIds: Set<string>; tab: 'transcript' | 'vocabulary' | 'favorites'; setTab: (tab: 'transcript' | 'vocabulary' | 'favorites') => void;
  ...
  onTranslate: (cueId: string) => void; onFavoriteToggle: (cue: Cue) => void; onFavoriteRemove: (id: string) => void; translatingCueIds: Set<string>; onReview: () => void; onNotify: (message: string) => void; translationLabel: string;
};
...
export function TranscriptPanel(props: Props) {
  ...
  return <section className="learning-panel" aria-label="双语技术文章">
    <div className="panel-tabs">
      ...
      <button className={props.tab === 'favorites' ? 'active' : ''} onClick={() => props.setTab('favorites')}><Bookmark size={15} />短句收藏 <span>{props.favoriteExamples.length}</span></button>
    </div>
    {props.tab === 'transcript' ? <>
      ...
      <div className="transcript-head"><span>时间</span><span>English</span><span>中文 · {props.translationLabel}</span><span>收藏</span></div>
      <div className="transcript-list" aria-live="polite">
        {filtered.map(({ cue, index }) => <article className={`cue-row ${index === props.activeIndex ? 'active' : ''}`} key={cue.id} onClick={() => { if (phraseSelectionRef.current) { phraseSelectionRef.current = false; return; } props.onCue(index); }}>
          ...
          <button className={`cue-favorite ${props.favoriteCueIds.has(cue.id) ? 'saved' : ''}`} type="button" disabled={!cue.zh || props.favoriteSavingCueIds.has(cue.id)} onClick={(event) => { event.stopPropagation(); props.onFavoriteToggle(cue); }} title={!cue.zh ? '请先翻译本句后再收藏' : props.favoriteCueIds.has(cue.id) ? '移除短句收藏' : '收藏短句'} aria-label={!cue.zh ? '请先翻译本句后再收藏' : props.favoriteCueIds.has(cue.id) ? '移除短句收藏' : '收藏短句'}>{props.favoriteSavingCueIds.has(cue.id) ? <LoaderCircle className="spin" size={15} /> : props.favoriteCueIds.has(cue.id) ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}</button>
    ...
    </> : props.tab === 'vocabulary' ? <div className="vocabulary-layout">
      ...
      <DictionaryWorkbench />
    </div> : <div className="favorite-examples">
      <div className="favorite-summary"><div><span>Saved Sentences</span><strong>短句收藏</strong><small>{props.favoriteExamples.length} 句值得反复回看的双语字幕</small></div><BookmarkCheck size={20} /></div>
      <div className="favorite-example-list">
        {props.favoriteExamples.map((item) => <article className="favorite-example" key={item.id}>
          <div className="favorite-example-copy"><p className="favorite-example-en">{item.sentence}</p><p className="favorite-example-zh">{item.translation}</p></div>
          <div className="favorite-example-meta"><span>课程来源 · {item.courseName}</span><span>{item.lessonTitle} · {formatTime(item.startSeconds)} · {sourceHost(item.sourceUrl)}</span></div>
          <button className="favorite-remove" type="button" onClick={() => props.onFavoriteRemove(item.id)} aria-label="移除短句收藏" title="移除短句收藏"><Trash2 size={15} /></button>
        </article>)}
        {!props.favoriteExamples.length && <div className="empty-review"><Bookmark /><strong>还没有收藏短句</strong><span>在双语字幕中翻译一句后，点击右侧书签即可保存。</span></div>}
      </div>
    </div>}
