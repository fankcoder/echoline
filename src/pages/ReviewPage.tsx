...
export default function ReviewPage() {
  ...
  useEffect(() => { if (current) void api<DictionaryEntry>(`/api/dictionary/${encodeURIComponent(current.word)}`).then(setEntry).catch(() => undefined); }, [current]);
  const finish = async () => { if (!deck.length) return; await api(`/api/review/${groupIndex}`, { method: 'POST', ...jsonBody({ words: deck.map((item) => item.word) }) }); await refresh(); notify(`第 ${groupIndex + 1} 组复习已记录`); reshuffle(); };
  return <main className="review-page"><header className="review-page-head"><button className="detail-command" onClick={() => navigate(data!.settings.currentLessonId ? `/learn/${data!.settings.currentLessonId}` : '/courses')}><ArrowLeft />返回学习</button><div><span className="eyebrow">Vocabulary Review</span><h1>生词复习</h1><p>每 10 个词为一组，每次进入组内顺序都会随机。</p></div></header>
    <section className="review-shell"><div className="review-toolbar"><div className="group-list">{groups.map((items, index) => <button className={index === groupIndex ? 'active' : ''} key={index} onClick={() => navigate(`/review/${index}`)}>第 {index + 1} 组 <small>{items.length}</small></button>)}</div><button className="reshuffle" onClick={reshuffle} aria-label="重新打乱"><RotateCcw /></button></div>
      {current ? <div className="review-session"><div className="review-progress"><span>第 {groupIndex + 1} 组</span><span>{position + 1} / {deck.length}</span></div><button className="flashcard" onClick={() => setRevealed(true)}><span className="flashcard-label">English</span><strong>{current.word}</strong><small>{entry?.word === current.word ? `${entry.ipa} ${entry.type}` : ''}</small>{revealed ? <div className="flashcard-answer"><p>{entry?.word === current.word ? entry.meaning : '正在查询本地词典…'}</p>{entry?.word === current.word && entry.note && <blockquote>{entry.note}</blockquote>}{entry?.word === current.word && entry.example && <em>{entry.example}</em>}</div> : <span className="reveal-hint">点击卡片查看释义</span>}</button><div className="review-actions"><button className="secondary-button" disabled={position === 0} onClick={() => { setPosition((value) => value - 1); setRevealed(false); }}><ChevronLeft />上一个</button>{position < deck.length - 1 ? <button className="next-button" onClick={() => { setPosition((value) => value + 1); setRevealed(false); }}>下一个<ChevronRight /></button> : <button className="next-button" onClick={() => void finish()}><BrainCircuit />完成本组</button>}</div></div> : <div className="empty-review"><BrainCircuit /><strong>这一组还没有单词</strong><span>先在字幕中点击单词加入生词本。</span></div>}
    </section>
  </main>;
}
